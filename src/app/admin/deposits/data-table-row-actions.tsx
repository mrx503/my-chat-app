...
import { handleDepositRequest } from "./actions";
...
export function DataTableRowActions<TData>({ row }: DataTableRowActionsProps<TData>) {
  const request = row.original as DepositRequest;
  ...
  const onAction = async () => {
    ...
    try {
      const result = await handleDepositRequest(request, action);
      if (result.success) {
        toast({
    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Are you sure you want to {action} this deposit request of {request.amount} coins for {request.email}? This action cannot be undone.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
